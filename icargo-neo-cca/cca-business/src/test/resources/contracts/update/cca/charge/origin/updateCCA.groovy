...
[
	Contract.make {
		...
		request {
			...
			body(file("saveCCAOriginRequest.json"))
		...
		response {
			...
			body([
			"shipment_prefix": "134",
			"master_document_number": "20000000",
	...
	Contract.make {
		...
		request {
			...
			body(file("updateOriginChargesRequest.json"))
			urlPath($(consumer(regex("(/cca/[0-9a-z]+/private/v1/cca/saveCCA)")), producer('/cca/av/private/v1/cca/saveCCA')))
			}
		response {
			status 200
	...
	Contract.make {
		...
		response {
			status 200
			body(file("updatedOriginChargesResponse.json"))
			headers {
				contentType(applicationJson())
			}
		}
