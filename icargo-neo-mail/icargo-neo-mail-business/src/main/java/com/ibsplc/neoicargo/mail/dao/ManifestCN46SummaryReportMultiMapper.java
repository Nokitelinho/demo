...
public class ManifestCN46SummaryReportMultiMapper implements MultiMapper<ConsignmentDocumentVO>{
	...
	public List<ConsignmentDocumentVO> map(ResultSet rs) throws SQLException {
		...
		while (rs.next()) {
			...
			if (rs.getTimestamp("CSGDAT") != null) {
				consignmentDocumentVO.setConsignmentDate(localDateUtil.getLocalDate(null,rs
						.getTimestamp("CSGDAT")));
			}
			...
			if(rs.getString(FSTFLTDEPDAT)!=null) {
				...
				consignmentDocumentVO.setFirstFlightDepartureDate(localDateUtil.getLocalDate(null,rs
						.getTimestamp(FSTFLTDEPDAT)));
			}
	...
	private MailInConsignmentVO populateMailInformation(MailInConsignmentVO mailInConsignmentVO, ResultSet rs) throws SQLException {
		...
		Quantity wgt=quantities.getQuantity(Quantities.MAIL_WGT, BigDecimal.valueOf(0.0),BigDecimal.valueOf(rs.getDouble("WGT")),UnitConstants.WEIGHT_UNIT_KILOGRAM);
		...
		if(rs.getString("LCWEIGHT")!=null) {
			mailInConsignmentVO.setTotalLetterWeight(quantities.getQuantity(Quantities.MAIL_WGT, BigDecimal.valueOf(0.0),BigDecimal.valueOf(rs.getDouble("LCWEIGHT")),UnitConstants.WEIGHT_UNIT_KILOGRAM));
		}
		if(rs.getString("CPWEIGHT")!=null) {
			mailInConsignmentVO.setTotalParcelWeight(quantities.getQuantity(Quantities.MAIL_WGT, BigDecimal.valueOf(0.0),BigDecimal.valueOf(rs.getDouble("CPWEIGHT")),UnitConstants.WEIGHT_UNIT_KILOGRAM));
		}
		if(rs.getString("EMSWEIGHT")!=null) {
			mailInConsignmentVO.setTotalEmsWeight(quantities.getQuantity(Quantities.MAIL_WGT, BigDecimal.valueOf(0.0),BigDecimal.valueOf(rs.getDouble("EMSWEIGHT")),UnitConstants.WEIGHT_UNIT_KILOGRAM));
		}
		if(rs.getString("SVWEIGHT")!=null) {
			mailInConsignmentVO.setTotalSVWeight(quantities.getQuantity(Quantities.MAIL_WGT, BigDecimal.valueOf(0.0),BigDecimal.valueOf(rs.getDouble("SVWEIGHT")),UnitConstants.WEIGHT_UNIT_KILOGRAM));
		}
	...
	private RoutingInConsignmentVO populateRoutingInformation(RoutingInConsignmentVO routingInConsignmentVO, ResultSet rs) throws SQLException {
		...
		if (rs.getTimestamp(FLTDAT) != null && rs.getString(FLTNUM)!=null) {
			routingInConsignmentVO.setOnwardFlightDate(localDateUtil.getLocalDate(null,rs
					.getTimestamp(FLTDAT)));
		}
