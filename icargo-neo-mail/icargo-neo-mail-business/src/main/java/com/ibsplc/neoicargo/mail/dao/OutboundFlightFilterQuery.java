...
public class OutboundFlightFilterQuery  extends PageableNativeQuery<MailAcceptanceVO> {
	...
	public OutboundFlightFilterQuery(int recordsPerPage, OperationalFlightVO operationalFlightVO, StringBuilder baseQuery1, String baseQuery2, boolean isOracleDataSource, OutboundFlightMapper mapper) {
	...
	public String getNativeQuery() {
		...
		ZonedDateTime flightDate = operationalFlightVO.getFlightDate();
		int carrierId =operationalFlightVO.getCarrierId();
		ZonedDateTime fromDate = operationalFlightVO.getFromDate();
		ZonedDateTime toDate = operationalFlightVO.getToDate();
		...
		if(flightDate!=null)
		{
			...
			this.setParameter(++index,flightDate.format(DateTimeFormatter.ofPattern("yyyy-MM-dd")));
		}
		if(flightSeqNum >0)
		{
		...
		if(fromDate!=null && !isFlightFilterPresent)
		{
			...
			setParameter(++index, LocalDateMapper.toLocalDate(fromDate));
		}

		if(toDate!=null && !isFlightFilterPresent)
		{
			stringBuilder.append(" AND FLTLEG.STD <= ? ");
			setParameter(++index, LocalDateMapper.toLocalDate(toDate));
