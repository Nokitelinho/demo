...
Contract.make {
    description('Should return partial shipment details by AWB\'s if some exist and some does not')
    request {
        method('GET')
        headers {
        ...
        urlPath($(consumer(regex("(/tracking/[0-9a-z]+/private/v1/shipments)")), producer('/tracking/av/private/v1/shipments'))) {
            queryParameters {
                parameter 'awb': value(consumer(matching("w+-{1}w+")), producer("134-7899"))
                parameter 'awb': value(consumer(matching("w+-{1}w+")), producer("1347899"))
