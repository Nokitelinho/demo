/*
 * RefreshULDDetailsTxnCommand.java Created on Feb 19, 2006
 *
 ...
 *
 */
public class RefreshULDDetailsTxnCommand  extends BaseCommand {
    ...
    */
    public void execute(InvocationContext invocationContext)
			throws CommandInvocationException {
		log.entering("RefreshULDDetailsTxnCommand", "execute");
		ListULDTransactionForm listULDTransactionForm = (ListULDTransactionForm) invocationContext.screenModel;

		ListULDTransactionSession listULDTransactionSession = getScreenSession(MODULE_NAME, SCREEN_ID);
		...
		if (transactionFilterVO != null) {
			...
			if (transactionFilterVO.getPageNumber() == 0) {
			...
			log.log(Log.FINE, "****txnListVO try**************************", txnListVO.getTransactionDetailsPage());
			if (txnListVO.getTransactionDetailsPage() != null && txnListVO.getTransactionDetailsPage().size() > 0) {
				for (ULDTransactionDetailsVO vo : txnListVO.getTransactionDetailsPage()) {
					if (vo.getMucIataStatus().equals(UPDATED) || vo.getMucIataStatus().equals(RESENT)
							|| vo.getMucIataStatus().equals(SENT)) {

						vo.setMucIataStatus(STR_GENERATED);
						log.log(Log.FINE, "vo.getMucIataStatus() ~~~~~~~~~~~~~~~~~", vo.getMucIataStatus());
		...
		listULDTransactionForm.setScreenStatusFlag(ComponentAttributeConstants.SCREEN_STATUS_SCREENLOAD);
		listULDTransactionForm.setPageURL(listULDTransactionSession.getPageURL());
