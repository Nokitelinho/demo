/*
 * CloseCommand.java Created on Feb 07,2006
 *
...
/**
 * This command class is invoked on the start up CloseCommand
 * 
 * @author A-1862
 */
	...
	public void execute(InvocationContext invocationContext)
		...
		//ADDED BY A-3251 SREEJITH P.C. FOR MAILTRACKING ANZ401.
			if (maintainDamageReportForm.getPageURL() != null
				&& ("FromScreenMAILACCEPTANCEFO").equals(maintainDamageReportForm.getPageURL())) {
			invocationContext.target = CLOSE_TOMAILACCEPTANCEFO;
			return;
		}
			if (maintainDamageReportForm.getPageURL() != null
					&& ("FromScreenMAILACCEPTANCEFC").equals(maintainDamageReportForm.getPageURL())) {
				invocationContext.target = CLOSE_TOMAILACCEPTANCEFC;
				return;
			}
			
			if (maintainDamageReportForm.getPageURL() != null
					&& ("FromScreenMAILACCEPTANCECO").equals(maintainDamageReportForm.getPageURL())) {
				invocationContext.target = CLOSE_TOMAILACCEPTANCECO;
				return;
			}
			
			if (maintainDamageReportForm.getPageURL() != null
					&& ("FromScreenMAILACCEPTANCECC").equals(maintainDamageReportForm.getPageURL())) {
		...
		if (maintainDamageReportForm.getPageURL() != null
				&& ("LISTDAMAGEREPORT").equals(maintainDamageReportForm.getPageURL())) {
			session.setScreenId("LISTDAMAGE");
			invocationContext.target = CLOSE_TOLISTDAMAGE;
			return;
		}
		if (maintainDamageReportForm.getPageURL() != null
				&& (("LoanBorrow").equals(maintainDamageReportForm.getPageURL())
						|| ("fromulderrorlogforborrowDamage").equals(maintainDamageReportForm.getPageURL())
						|| ("fromScmReconcileBorrowDamage").equals(maintainDamageReportForm.getPageURL())
						|| ("fromulderrorlogforloanDamage").equals(maintainDamageReportForm.getPageURL()) 
						|| "fromScmUldReconcileDamage".equals(maintainDamageReportForm.getPageURL()))) {
		...
		if (maintainDamageReportForm.getPageURL() != null
				&& ("MidLoanBorrow").equals(maintainDamageReportForm.getPageURL())) {
			invocationContext.target = CLOSE_TOMIDLOANBORROW;
			return;
		}
		if (maintainDamageReportForm.getPageURL() != null
				&& ("ReturnLoanBorrow").equals(maintainDamageReportForm.getPageURL())) {
			invocationContext.target = CLOSE_TORETURNLOANBORROW;
			return;
		}
		if (maintainDamageReportForm.getPageURL() != null&& ("LISTULD").equals(maintainDamageReportForm.getPageURL())) {
		...
		if (canSave) {
			...
			ErrorVO error = new ErrorVO(
					"uld.defaults.maintainDmgRep.msg.err.dmgnotsaved");
			error.setErrorDisplayType(ErrorDisplayType.WARNING);
