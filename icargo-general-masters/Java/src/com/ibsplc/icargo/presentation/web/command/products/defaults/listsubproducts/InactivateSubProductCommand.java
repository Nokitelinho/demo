...
/**
 * InactivateSubProductCommand is for inactivating a subproduct
 * @author A-1870
 *
 */
public class InactivateSubProductCommand extends BaseCommand{
	...
	throws CommandInvocationException {
		...
		if(errors!=null && errors.size()>0){
			...
			invocationContext.target = "subinactivate_failure";
			return;
		}
		String productCode="";
		try{
		...
		invocationContext.target ="subinactivate_success";
	}
			...
			for(SubProductVO subProductVo:pageSubProductVO){
				if(chk[0].equals(subProductVo.getSubProductCode())){
					if(INACTIVE.equals(subProductVo.getStatus())){
					     ...
					     error = new ErrorVO("products.defaults.cannotinactivatesubproduct");
					     error.setErrorDisplayType(ErrorDisplayType.ERROR);
					     errors.add(error);
				    }
					else if(NEW.equals(subProductVo.getStatus())){

						 error = new ErrorVO("products.defaults.cannotinactivatesubproductofstatusnew");
						 error.setErrorDisplayType(ErrorDisplayType.ERROR);
						 errors.add(error);
				    }
					break;
			 ...
			 error = new ErrorVO("products.defaults.cannotinactivatesubproductmorethanone");
			 error.setErrorDisplayType(ErrorDisplayType.ERROR);
			 errors.add(error);
		}
