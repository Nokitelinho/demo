/*
 * DeleteDamageDetailsCommand.java Created on Feb 07, 2006
 *
                ...
                if (index == Integer.parseInt(selectedRows[i])) {
        		   ...
        		   if(uldDamagevo.getOperationFlag()!=null &&
        				   	uldDamagevo.getOperationFlag().equals
        								(AbstractVO.OPERATION_FLAG_INSERT))
        		   {
        			   uldDamagevo.setOperationFlag(OPERATION_FLAG_INS_DEL);
        		   }
...
log.log(Log.FINE,
               ...
               if(uldDamageVO!=null && uldDamageVO.size()>0){
					 for(ULDDamageVO uldDamageVo:uldDamageVO){
						 ...
						 int damagePoints=Integer.parseInt(uldDamageVo.getDamagePoints());
						if(uldDamageVo.getOperationFlag()==null && damagePoints!=0 ){
		...
		maintainDamageReportSession.getULDDamageVO()
         									.setUldDamageVOs(uldDamageVOtmp);
		...
		boolean isDmgPresent=false;
		 ...
		 for(ULDDamageVO uldDamageVOafter : uldDamageVOAfter) {
			 if(uldDamageVOafter.getOperationFlag()==null 
					 || (AbstractVO.OPERATION_FLAG_INSERT).equals(uldDamageVOafter.getOperationFlag()) 
					 ||	(("U").equals(uldDamageVOafter.getOperationFlag())&& !uldDamageVOafter.isClosed())){
				 dmgFlag = true;
				 break;
			 }			 
		 }
