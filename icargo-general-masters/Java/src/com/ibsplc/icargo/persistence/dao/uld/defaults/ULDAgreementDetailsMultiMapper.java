/*
 * ULDAgreementDetailsMultiMapper.java Created on Dec 19, 2005
 *
 ...
 *
 */
public class ULDAgreementDetailsMultiMapper implements MultiMapper<ULDAgreementVO>{
	  private Log log=LogFactory.getLogger(" ULD_DEFAULTS");

	  private String airportCode = null;
	  public ULDAgreementDetailsMultiMapper(String logonAirportCode){
		  airportCode = logonAirportCode;
	  }
   /**
    * @param rs
    * @return List<ULDAgreementVO>
    * @throws SQLException
    */
    public List<ULDAgreementVO> map(ResultSet rs) throws SQLException{
    	...
    	Collection <ULDAgreementDetailsVO> uldAgreementDetailsVOs = null;
		...
		sbOne = new StringBuilder();
    		childPresId = sbOne.append(presId).append(rs.getInt("SEQNUM")).toString();
    ...
    if(uldAgreementVO != null){
		uldAgreementVO.setUldAgreementDetailVOs(uldAgreementDetailsVOs);
		uldAgreementVOs.add(uldAgreementVO);
	}

	log.exiting("ULDAgreementDetailsMultiMapper","map");
