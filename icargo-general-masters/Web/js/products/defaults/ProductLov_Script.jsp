...
*/
	...
	function onClickList(){
		...
		submitForm(frm,'products.defaults.screenloadProductLov.do')
	}
	...
	function closeScreen(frm,strAction,actn,nextAction){
		...
		window.opener.document.forms[1].action=nextAction;
	...
	function submitFrm(codeObject,formNum)
	{
		...
		if(document.forms[0].elements.productCodeField.value!=""){
			var prdCodeField = eval("window.opener.document.forms["+formNum+"]."+document.forms[0].elements.productCodeField.value);
			prdCodeField.value=prdCode;
			}
		}else{

			var rowCnt = rowIndex.value;
			var objCode = eval("window.opener.targetFormName.elements."+codeObject);
			objCode[rowCnt].value=str;
			if(targetFormName.elements.productCodeField.value!=""){
				var prdCodeField = eval("window.opener.targetFormName"+targetFormName.elements.productCodeField.value);
	...
	if(window.opener) {
				if(invokeFunction!=null){
					try{
						eval("window.opener."+invokeFunction);
					}
					catch(e){
						//TODO
					}
...
function submitFrmForBooking(codeObject,priorityObj,tranModeObj,formNum){
     ...
     for(var i=0;i<document.forms[0].elements.length;i++)
     		...
     		if(document.forms[0].elements.productCodeField.value!=""){
     			//Modified by A-1927 @ NRT on 12-Jul-2007 for NCA Bug Fix starts
     			var prdCodeField = eval("window.opener.targetFormName.elements.productCode.value");
     			window.opener.targetFormName.elements.productCode.value=prdCode;
     			//Modified by A-1927 @ NRT on 12-Jul-2007 for NCA Bug Fix ends
...
function callFunction(lastPg,displayPg){
	...
	document.forms[0].elements.bookingDate.value = bookingDate;
	document.forms[0].elements.activeProducts.value = activeProducts;
	document.forms[0].action="products.defaults.screenloadProductLov.do";
	document.forms[0].elements.productObject.value = codeName;
...
function clearAction() {
	targetFormName.action = "products.defaults.clearProductLovScreen.do";
