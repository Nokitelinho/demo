/*
 * SearchContainerFilterQuery.java Created on Jun 14, 2006
 *
 ...
 *
 * @author a-1936
 * @author A-1936 Modified By Karthick V to incorporate the Carrier\Destintaion changes as the
 * Part of the NCA Mail Tracking CR
 * Also the Transfer Changes ..
 *
 *
 */
...
public class SearchContainerFilterQuery extends PageableNativeQuery<ContainerVO> {
	...
	/* public SearchContainerFilterQuery(
			SearchContainerFilterVO searchContainerFilterVO, String baseQuery)
			throws SystemException {
		 super();
	...
	public SearchContainerFilterQuery(String baseQuery,SearchContainerFilterVO searchContainerFilterVO,Mapper<ContainerVO> mapper,boolean isOracleDataSource) throws SystemException{
		super(searchContainerFilterVO.getTotalRecords(), mapper);
		this.searchContainerFilterVO = searchContainerFilterVO;
        this.baseQuery = baseQuery;
	...
	//added by A-5201 for CR ICRD-21098
	public SearchContainerFilterQuery(String baseQuery,SearchContainerFilterVO searchContainerFilterVO,MultiMapper<ContainerVO> multimapper,boolean isOracleDataSource) throws SystemException{
    ...
    //Added by A-7929 as part of ICRD-320470
	 public SearchContainerFilterQuery(String baseQuery, SearchContainerFilterVO searchContainerFilterVO,
			ContainerMapperForDestination mapper,boolean isOracleDataSource, String fromScreen) throws SystemException {
		   super(searchContainerFilterVO.getPageSize(),-1,mapper);
			this.searchContainerFilterVO = searchContainerFilterVO;
			this.baseQuery = baseQuery;
			this.isOracleDataSource = isOracleDataSource;
	}
	...
	public SearchContainerFilterQuery(String baseQuery, SearchContainerFilterVO searchContainerFilterVO,
	 ...
	 public String getNativeQuery() {
			 ...
			 LocalDate fromDate = searchContainerFilterVO.getAssignedFromDate();
			 LocalDate toDate = searchContainerFilterVO.getAssignedToDate();
			 LocalDate flightdate = searchContainerFilterVO.getFlightDate();
		...
		if((MailConstantsVO.SEARCH_MODE_ALL .equals(searchMode) ||MailConstantsVO.SEARCH_MODE_FLIGHT.equals(searchMode)) &&
		     ...
		     log.log(Log.FINE," Append  Query  for  Transfer  Status");
		...
		if (fromDate != null) {
			 ...
			 this.setParameter(++index, Integer.parseInt(fromDate.toStringFormat(MailConstantsVO.YYYY_MM_DD).substring(0, 8)));
		}

		if (toDate != null) {
			 builder.append(" AND  TO_NUMBER (to_char (MST.ASGDAT, 'YYYYMMDD')) <= ? ");
			 this.setParameter(++index, Integer.parseInt(toDate.toStringFormat(MailConstantsVO.YYYY_MM_DD).substring(0, 8)));
		}
//added by A-5945 for ICRD-96261
		...
		if (flightdate != null) {
			 ...
			 this.setParameter(++index, Integer.parseInt(flightdate.toStringFormat(MailConstantsVO.YYYY_MM_DD).substring(0, 8)));
		}
...
//			.append(" AND MALMST.MALSTA  IN ('ARR','DLV'))) " );
		 ...
		 .append(" MST.OFLFLG,  MST.ARRSTA, MST.TRAFLG, MST.LSTUPDTIM,   MST.LSTUPDUSR, MST.CONJRNIDR, MST.POACOD ,  MST.TRNFLG ,MST.CNTIDR, MST.ACTULDWGT, MST.ACTULDWGTDSP , "
		 		+ "MST.ACTULDWGTDSPUNT , MST.ULDFULIND,MST.ACTWGTSTA, MST.ULDREFNUM,SYSPAR.PARVAL,");
		...
		if (MailConstantsVO.SEARCH_MODE_ALL.equals(searchMode) &&
			...
			builder.append("UNION")
					...
					.append(" '' TRAFLG ,MST.LSTUPDTIM,MST.LSTUPDUSR,MST.CONJRNIDR, MST.POACOD SBCODE,MST.TRNFLG,MST.CNTIDR,MST.ACTULDWGT, MST.ACTULDWGTDSP , MST.ACTULDWGTDSPUNT,MST.ULDFULIND,MST.ACTWGTSTA, NULL POL,'' ONWFLTDAT,")
			...
			//	if(destionationCarrierId>0){
				  if(finalDestination != null && finalDestination.trim().length() > 0) {
					  log.log(Log.INFO,"FINAL DESTINATION");
						builder.append("  AND MST.DSTCOD=? ");
						this.setParameter(++index, finalDestination);
				  }
		//		}
...
//Added by A-5945 for ICRD-96261
				...
				.append("MST.CONJRNIDR, MST.POACOD , MST.TRNFLG,  MST.LSTUPDTIM , MST.LSTUPDUSR,MST.CNTIDR, MST.ACTULDWGT , MST.ACTULDWGTDSP , MST.ACTULDWGTDSPUNT,MST.ULDFULIND,MST.ACTWGTSTA ,MST.ULDREFNUM,SYSPAR.PARVAL ");
		...
		/**
	 * To override in the corresponding child class
	 * @param builder
	 */
	...
	public static SearchContainerFilterQuery  getInstance(String baseQuery, SearchContainerFilterVO searchContainerFilterVO,boolean isOracleDataSource) throws SystemException{
		SearchContainerFilterQueryBuilder searchContainerFilterQuery =
				(SearchContainerFilterQueryBuilder) SpringAdapter.getInstance().getBean("mail.operations.serachContainerFilterQueryBuilder");		
		return searchContainerFilterQuery.withBaseQuery(baseQuery).withIsOracleDataSource(isOracleDataSource)
