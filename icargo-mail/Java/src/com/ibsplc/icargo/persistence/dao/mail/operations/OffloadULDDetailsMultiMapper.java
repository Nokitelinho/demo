...
public class OffloadULDDetailsMultiMapper implements MultiMapper<OffloadULDDetailsVO> {
    ...
    public List<OffloadULDDetailsVO> map(ResultSet rs) throws SQLException {
        ...
        while(rs.next()) {
            uldPK = rs.getString(CMPCOD) + rs.getString(FLTNUM) + rs.getString(FLTCARIDR) + rs.getString(FLTSEQNUM) + rs.getString(ULDNUM) + rs.getString(LEGORG);
            ...
            if(rs.getString(MSTDOCNUM) != null) {
                shipmentPK = rs.getString(CMPCOD) + rs.getString(LEGORG) + rs.getString(FLTCARIDR) + rs.getString(FLTNUM) + rs.getString(FLTSEQNUM) + rs.getString(ULDNUM) + rs.getString(MSTDOCNUM) + rs.getString("DOCOWRIDR") + rs.getString("SEQNUM") + rs.getString("DUPNUM");
    ...
    private void setFlightTimes(ResultSet rs, OffloadULDDetailsVO offloadULDDetailsVO) throws SQLException {
        if(rs.getTimestamp("ATD") != null) {
            if(rs.getString(LEGORG) != null) {
                offloadULDDetailsVO.setAtd(new LocalDate(rs.getString(LEGORG), Location.ARP, rs.getTimestamp("ATD")));
            } else {
                offloadULDDetailsVO.setAtd(new LocalDate("***", Location.NONE, rs.getTimestamp("ATA")));
            }
        }

        if(rs.getTimestamp("ETD") != null) {
            if(rs.getString(LEGORG) != null) {
                offloadULDDetailsVO.setEtd(new LocalDate(rs.getString(LEGORG), Location.ARP, rs.getTimestamp("ETD")));
            } else {
                offloadULDDetailsVO.setEtd(new LocalDate("***", Location.NONE, rs.getTimestamp("ETD")));
            }
        }

        if(rs.getTimestamp("STD") != null) {
            if(rs.getString(LEGORG) != null) {
                offloadULDDetailsVO.setStd(new LocalDate(rs.getString(LEGORG), Location.ARP, rs.getTimestamp("STD")));
            } else {
                offloadULDDetailsVO.setStd(new LocalDate("***", Location.NONE, rs.getTimestamp("STD")));
            }
        }
