...
public class MailBookingMapper implements MultiMapper<MailBookingDetailVO>   {
	...
	public List<MailBookingDetailVO> map(ResultSet rs) throws SQLException {
		...
		if(mailBookingMap.containsKey(key))
		{
	        ...
	        if(carrierCode!=null && carrierCode.trim().length()>0){
	        	...
	        	if(nextFlightNum!=null && nextFlightNum.trim().length()>0){//added by A-7371 as part of ICRD-249374
	        		newFlight=new StringBuilder().append(mailBookingNewVO.getSelectedFlightNumber()).append(",").append(nextFlightNum).append(" ").append(secondDate).append(" ").append(rs.getString("FLTCARCOD")).append(" ").append(rs.getInt("FLTCARIDR"))
	        												.append(" ").append(rs.getInt("FLTSEQNUM")).append(" ").append(rs.getString("FLTORG")).append(" ").append(rs.getString("FLTDST")).toString();
	        	}else{
			     newFlight=new StringBuilder().append(mailBookingNewVO.getSelectedFlightNumber()).append(",").append(" ").append(secondDate).append(" ").append(rs.getString("FLTCARCOD"))
			    		 .append(" ").append(rs.getInt("FLTCARIDR")).append(" ").append(rs.getInt("FLTSEQNUM")).append(" ").append(rs.getString("FLTORG")).append(" ").append(rs.getString("FLTDST")).toString();
		...
		{
			bookedFlights =  new ArrayList<MailBookingDetailVO>() ;
			bookedFlight = new MailBookingDetailVO();
			...
			if(mailBookingFlightDetailVO.getBookingFlightNumber()!=null && mailBookingFlightDetailVO.getBookingFlightNumber().trim().length()>0){
			 firstFlightDetails=new StringBuilder().append(mailBookingFlightDetailVO.getBookingFlightNumber()).append(" ").append(mailBookingFlightDetailVO.getBookingFlightDate().toDisplayFormat("dd-MMM-yyyy")).append(" ")
					 	.append(rs.getString("FLTCARCOD")).append(" ").append(rs.getInt("FLTCARIDR")).append(" ").append(rs.getInt("FLTSEQNUM")).append(" ").append(rs.getString("FLTORG")).append(" ").append(rs.getString("FLTDST")).toString();
			}else{
				 firstFlightDetails=new StringBuilder().append(" ").append(mailBookingFlightDetailVO.getBookingFlightDate().toDisplayFormat("dd-MMM-yyyy")).append(" ")
						 .append(rs.getString("FLTCARCOD")).append(" ").append(rs.getInt("FLTCARIDR")).append(" ").append(rs.getInt("FLTSEQNUM")).append(" ").append(rs.getString("FLTORG")).append(" ").append(rs.getString("FLTDST")).toString();
			}
