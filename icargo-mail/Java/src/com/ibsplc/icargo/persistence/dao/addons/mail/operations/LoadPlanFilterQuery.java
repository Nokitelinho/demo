...
public class LoadPlanFilterQuery extends PageableNativeQuery<MailBookingDetailVO>{
	
	private MailBookingFilterVO mailBookingFilterVO;
	private String baseQuery;
	
	public LoadPlanFilterQuery(LoadPlanMapper mapper,
	...
	public String getNativeQuery() {
		...
		int index = 0;
		setParameter(++index, mailBookingFilterVO.getPlannedFlightDateFrom().toDisplayDateOnlyFormat());
		setParameter(++index, mailBookingFilterVO.getPlannedFlightDateTo().toDisplayDateOnlyFormat());
		...
		if(!StringUtils.isBlank(mailBookingFilterVO.getFlightNumber())){
			queryString.append("AND PLNSHP.FLTNUM = ? ");
			setParameter(++index, mailBookingFilterVO.getFlightNumber());
		}
		if(mailBookingFilterVO.getFlightDate()!=null){
			queryString.append("AND TO_NUMBER(TO_CHAR(TRUNC(LEG.STD), 'YYYYMMDD')) = TO_NUMBER(TO_CHAR(TO_DATE(?), 'YYYYMMDD')) ");
		...
		if(!StringUtils.isBlank(mailBookingFilterVO.getPou())){
			queryString.append("AND PLNSHP.SEGDST = ? ");
