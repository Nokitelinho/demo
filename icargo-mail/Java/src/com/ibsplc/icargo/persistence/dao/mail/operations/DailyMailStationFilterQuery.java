/*
 * DailyMailStationFilterQuery.java Created on MAR 17, 2008
...
public class DailyMailStationFilterQuery extends NativeQuery {
	...
	public String getNativeQuery() {
		log.entering("DailyMailStationFilterQuery", "getNativeQuery");
		int index = 0;
	    ...
	    //LocalDate flightDate=filterVO.getFilghtDate();
  		 //Date flightDat=null;
		LocalDate flightFromDate=filterVO.getFlightFromDate();
		LocalDate flightToDate=filterVO.getFlightToDate();
		Date flightFromDat=null;
		Date flightToDat=null;
  		 	if (flightFromDate != null) {
  		 		flightFromDat = flightFromDate.toSqlDate();
  		 	}
  		 	if (flightToDate != null) {
  		 		flightToDat = flightToDate.toSqlDate();
  		 	}
...
/* 	        qry.append(" UNION ALL  SELECT   (SELECT TWOAPHCOD FROM SHRARLMST WHERE CMPCOD = FLTSEG.CMPCOD AND ARLIDR = FLTSEG.FLTCARIDR) CARCOD,FLTSEG.FLTNUM FLTNUM, ");
			...
			if((filterVO.getFlightNumber()!=null)&&(!"".equals(filterVO.getFlightNumber().trim()))){
 			 ...
 			 log.log(Log.FINE, "!!!!!######-->Query Generated After All Filter",
					qry);
			log.exiting("DailyMailStationFilterQuery", "getNativeQuery");
			return qry.toString();
	}
}
