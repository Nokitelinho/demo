...
public class SecurityScreeningValidationFilterQuery extends NativeQuery {
	...
	private static final Log LOGGER= LogFactory.getLogger("MAILTRACKING DEFAULTS");
	...
	public String getNativeQuery() {
		LOGGER.entering("SecurityScreeningValidationFilterQuery", "getNativeQuery");
	...
	private void appendSuffix(StringBuilder queryBuilder, StringBuilder appendNotNullCondition) {
		...
		LOGGER.exiting("SecurityScreeningValidationFilterQuery", "getNativeQuery");
	}
	...
	private int populateFilterValueForAppRegVal(StringBuilder queryBuilder, boolean appendAnd,
		...
		if(securityScreeningValidationFilterVO.getAppRegTransistCountry()!=null){
			appendAndOrForFilter(queryBuilder, appendAnd, appendNotNullCondition);
			queryBuilder.append("(COALESCE(APLREGTRSCNTFLG,'Y') = 'Y' AND (INSTR(','||APLREGTRSCNT||',', ','||?||',')=0) ")
			.append("OR (COALESCE(APLREGTRSCNTFLG,'Y') = 'N' AND (INSTR(','||APLREGTRSCNT||',', ','||?||',')>0)))");
		...
		if(securityScreeningValidationFilterVO.getAppRegTransistCountryGroup()!=null){
			appendAndOrForFilter(queryBuilder, appendAnd, appendNotNullCondition);
			queryBuilder.append("(COALESCE(APLREGTRSCNTGRPFLG,'Y') = 'Y' AND (INSTR(','||APLREGTRSCNTGRP||',', ','||?||',')=0) ")
			.append("OR (COALESCE(APLREGTRSCNTGRPFLG,'Y') = 'N' AND (INSTR(','||APLREGTRSCNTGRP||',', ','||?||',')>0)))");
