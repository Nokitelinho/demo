...
public class ExportFlightOperationsMultiMapper implements MultiMapper<ManifestVO> {
	...
	public List<ManifestVO> map(ResultSet rs) throws SQLException {
		...
		while (rs.next()) {
			...
			flightPK = new StringBuilder().append(rs.getString(CMPCOD))
					.append(rs.getString(LEGDST))
					.append(rs.getString(FLTCARIDR))
					.append(rs.getString(FLTNUM))
					.append(rs.getString(FLTSEQNUM)).toString();
			...
			uldPK = new StringBuilder().append(rs.getString(CMPCOD))
					.append(rs.getString(LEGDST))
					.append(rs.getString(FLTCARIDR))
					.append(rs.getString(FLTNUM))
					.append(rs.getString(FLTSEQNUM))
	...
	private void populateManifestedUldDetails(UldManifestVO uldManifestVO, ResultSet rs) throws SQLException {
		...
		uldManifestVO.setGrossWeight(new Measure(UnitConstants.WEIGHT, rs.getDouble(GRSWGT)));
		uldManifestVO.setTareWeight(new Measure(UnitConstants.WEIGHT, rs.getDouble(TARWGT)));
		uldManifestVO.setRemarks(rs.getString(ULDRMK));
		uldManifestVO.setSegmentSerialNumber(rs.getInt(SEGSERNUM));
		uldManifestVO.setNetWeight(new Measure(UnitConstants.WEIGHT, rs.getDouble(NETWGT)));
		uldManifestVO.setManifestedPieces(rs.getInt(MFTPCS));
		uldManifestVO
				.setManifestedWeight(new Measure(UnitConstants.WEIGHT, rs.getDouble(MFTWGT)));
	...
	private void updateManifestedUldDetails(UldManifestVO uldManifestVO, ResultSet rs) throws SQLException {
		...
		uldManifestVO
				.setManifestedPieces(uldManifestVO.getManifestedPieces() + rs.getInt(MFTPCS));
		double manifestSum = uldManifestVO.getManifestedWeight().getSystemValue()
				+ rs.getDouble(MFTWGT);
		uldManifestVO.setManifestedWeight(new Measure(UnitConstants.WEIGHT, manifestSum));
	...
	private ManifestVO populateExportFlightOperationsVO(ResultSet rs) throws SQLException {
		...
		String fltCarrierCode = rs.getInt(APHCODUSE) == 2 ? rs.getString(TWOAPHCOD) : 
			rs.getString(THRAPHCOD);
	...
	private void populateFlightDateAndTimeDetails(FlightDetailsVO flightDetailsVO, ResultSet rs) throws SQLException{
		if (rs.getTimestamp(FLTDAT) != null) {
			LocalDate date = rs.getString(LEGORG) != null
					? new LocalDate(rs.getString(LEGORG), Location.ARP,
							rs.getTimestamp(FLTDAT))
					: new LocalDate(LocalDate.NO_STATION, Location.NONE,
							rs.getTimestamp(FLTDAT));
			flightDetailsVO.setFlightDate(date);
		}
		if (rs.getTimestamp(FLTATD) != null) {
			LocalDate atd = rs.getString(LEGORG) != null
					? new LocalDate(rs.getString(LEGORG), Location.ARP,
							rs.getTimestamp(FLTATD))
					: new LocalDate(LocalDate.NO_STATION, Location.NONE,
							rs.getTimestamp(FLTATD));
			flightDetailsVO.setAtd(atd);
		}

		if (rs.getTimestamp(FLTETD) != null) {
			LocalDate etd = rs.getString(LEGORG) != null
					? new LocalDate(rs.getString(LEGORG), Location.ARP,
							rs.getTimestamp(FLTETD))
					: new LocalDate(LocalDate.NO_STATION, Location.NONE,
							rs.getTimestamp(FLTETD));
			flightDetailsVO.setEtd(etd);
		}

		if (rs.getTimestamp(FLTSTD) != null) {
			LocalDate std = rs.getString(LEGORG) != null
					? new LocalDate(rs.getString(LEGORG), Location.ARP,
							rs.getTimestamp(FLTSTD))
					: new LocalDate(LocalDate.NO_STATION, Location.NONE,
							rs.getTimestamp(FLTSTD));
			flightDetailsVO.setStd(std);
		}
		if(Objects.nonNull(rs.getTimestamp(FOWMILTIM))){
			LocalDate fowMilestoneTime = new LocalDate(rs.getString(LEGORG), Location.ARP,
					rs.getTimestamp(FOWMILTIM));
			flightDetailsVO.setFowMilestoneTime(fowMilestoneTime);
		}
	}
}
