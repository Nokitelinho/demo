 ...
 *
 */
public class ResolveExceptionCommand extends BaseCommand {
   ...
   public void execute(InvocationContext invocationContext)
		...
		if (finalMailUploadVOsForSave != null) {
			for (MailUploadVO mailUploadVOForSave : finalMailUploadVOsForSave) {
				...
				//mailUploadVOs.add(mailUploadVOForSave);
				  try {
					  if(errorHandlingPopUpForm.getTransactionName().equals("getManifestInfo"))
					  
						  mailUploadVOForSave.setProcessPoint("ARR");
					  mailUploadVOForSave.setResolveFlagAndroid("Y");
					  if(mailUploadVOForSave.isArrivalException()){//Added by A-8164 for ICRD-330543
						  mailUploadVOForSave.setFromErrorHandling(true);
					  }
					  if(mailUploadVOForSave.isForceAccepted()){
						  mailUploadVOForSave.setFromErrorHandlingForForceAcp(true); 
					  }
						scannedMailDetailsVO = mailTrackingDefaultsDelegate
								.saveMailUploadDetailsForAndroid(mailUploadVOForSave,
										mailUploadVO.getScannedPort());
						/*Added by A-8353 for IASCB-34167 
						 *  calling again  in order to throw exception again without breaking the arrival 
						 *  flow so stamping of  exception  will happen in pol port in error handling screen
						 */
						if(scannedMailDetailsVO.isArrivalException()){  
							if(mailUploadVOForSave.isArrivalException()){
								  mailUploadVOForSave.setResolveFromErrorHandling(true);
								  mailUploadVOForSave.setFlightDate(scannedMailDetailsVO.getFlightDate());
								  mailUploadVOForSave.setFlightNumber(scannedMailDetailsVO.getFlightNumber());
								  mailUploadVOForSave.setCarrierCode(scannedMailDetailsVO.getCarrierCode());
								  mailUploadVOForSave.setContainerPOU(scannedMailDetailsVO.getPol());
								  mailUploadVOForSave.setScannedPort(scannedMailDetailsVO.getPol());
								  mailUploadVOForSave.setContainerNumber(scannedMailDetailsVO.getContainerNumber());    
								  mailUploadVO.setScannedPort(scannedMailDetailsVO.getPol());
							  }
								scannedMailDetailsVO = mailTrackingDefaultsDelegate
										.saveMailUploadDetailsForAndroid(mailUploadVOForSave,
												mailUploadVO.getScannedPort());
			...
			log.log(Log.FINE, "***values to be saved*******", mailUploadVOForSave);
	...
	private Collection<MailUploadVO> validateAndUpdateMailUploadVOs(
		...
		identifyAndUpdateNewChangesOfErrorVO(errorHandlingPopUpForm,
				oldMailUploadVO);   
		//mailUploadMap.remove(errorHandlingPopUpForm.getSelectedtxnid()); //Commented by A-8164 for ICRD-330543
		Collection<MailUploadVO> finalMailUploadVOsForSave = new ArrayList<MailUploadVO>(); 
		//finalMailUploadVOsForSave.add(oldMailUploadVO);//Commented by A-8164 for ICRD-330543
	...
	private void identifyAndUpdateNewChangesOfErrorVO(
		...
		if (mailbagID != null && mailbagID.trim().length() == 29) {
			...
			if(mailUploadVO.getMailTag()!=null && 
					!mailUploadVO.getMailTag().equals(errorHandlingPopUpForm.getMailBag().toUpperCase())){
				errorHandlingPopUpForm.setIsMailTagChanged(MailConstantsVO.FLAG_YES);
			}
			...
			if ((errorHandlingPopUpForm.getYear().length()>0 && mailUploadVO.getYear() != Integer
			...
			if (mailUploadVO.getOrgin()!= null
					&& !mailUploadVO.getOrgin().equals(
							errorHandlingPopUpForm.getOrgin().toUpperCase())) {
				errorHandlingPopUpForm.setIsOriginChanged(MailConstantsVO.FLAG_YES);
			...
			if (mailUploadVO.getDestination()!= null
					&& !mailUploadVO.getDestination().equals(
							errorHandlingPopUpForm.getDestination().toUpperCase())) {
				errorHandlingPopUpForm.setIsDestnChanged(MailConstantsVO.FLAG_YES);
		...
		else if(isValidMailtag(mailUploadVO.getMailTag().trim().length())){//7531
			if (mailUploadVO.getOrgin()!= null
					&& !mailUploadVO.getOrgin().equals(
							errorHandlingPopUpForm.getOrgin().toUpperCase())) {
				errorHandlingPopUpForm.setIsOriginChanged(MailConstantsVO.FLAG_YES);
			...
			if (mailUploadVO.getMailTag()!= null
					&& !mailUploadVO.getMailTag().equals(
							errorHandlingPopUpForm.getMailBag().toUpperCase())) {
				errorHandlingPopUpForm.setIsMailTagChanged(MailConstantsVO.FLAG_YES);
			}
		}
	...
	private void updateSelectedmailUploadVO(MailUploadVO mailUploadVO,
		...
		if (mailUploadVO != null) {
			...
			mailUploadVO.setContainerNumber(errorHandlingPopUpForm
				...
				if (errorHandlingPopUpForm.getDoe() != null
						&& errorHandlingPopUpForm.getDoe().trim().length() > 4
						&& !mailUploadVO.getScanType().equals(
								MailConstantsVO.MAIL_STATUS_ACCEPTED) 
						&&!MailConstantsVO.MAIL_STATUS_ARRIVED.equals(mailUploadVO.getScanType())
										) {
			...
			if (errorHandlingPopUpForm.getPol() != null
				...
				{
				    if(errorHandlingPopUpForm.getOoe().length()==6)
					mailUploadVO.setContainerPol(errorHandlingPopUpForm.getOoe().substring(2, 5).toUpperCase());
				    else
				    	mailUploadVO.setContainerPol(errorHandlingPopUpForm.getOoe().toUpperCase());
				    	//Modified as part of IASCB-42613
				}
		}
		...
		if(errorHandlingPopUpForm.getMailBag()==null ){
			...
			if (errorHandlingPopUpForm.getOrgin() != null
					&& errorHandlingPopUpForm.getOrgin().trim().length() != 0) {
				mailUploadVO.setOrgin(errorHandlingPopUpForm
						.getOrgin().toUpperCase());
			}
			if (errorHandlingPopUpForm.getDestn() != null
					&& errorHandlingPopUpForm.getDestn().trim().length() != 0) {
				mailUploadVO.setDestination(errorHandlingPopUpForm
						.getDestn());
			}
			if (errorHandlingPopUpForm.getMailBag() != null
					&& errorHandlingPopUpForm.getMailBag().trim().length() != 0) {
				mailUploadVO.setMailTag(errorHandlingPopUpForm
						.getMailBag());
			}
			if (errorHandlingPopUpForm.getOoe() != null
					&& errorHandlingPopUpForm.getOoe().trim().length() != 0) {
	...
	private void updateMailUploadVO(MailUploadVO currentMailUploadVO,
			...
			ErrorHandlingPopUpForm errorHandlingPopUpForm) {
		LogonAttributes logonAttributes = getApplicationSession().getLogonVO();
		String mailbagID = currentMailUploadVO.getMailTag().toUpperCase();
		if (mailbagID != null && mailbagID.trim().length() == 29) {
		...
		//Modified as part of code quality work by A-7531 starts
		/*if ((currentMailUploadVO.getCarrierCode() != null && currentMailUploadVO //Commented by A-8164 for ICRD-330543
				.getCarrierCode().trim().length() > 0)
		...
		if (currentMailUploadVO.getMailTag() != null
			...
			if(MailConstantsVO.FLAG_YES.equals(errorHandlingPopUpForm
					.getIsMailTagChanged())){
				currentMailUploadVO.setMailTag(errorHandlingPopUpForm.getMailBag().toUpperCase());
				errorHandlingPopUpForm.setIsMailTagChanged(MailConstantsVO.FLAG_YES);
					}
			if (MailConstantsVO.FLAG_YES.equals(errorHandlingPopUpForm
					.getIsOOEChanged())) {
