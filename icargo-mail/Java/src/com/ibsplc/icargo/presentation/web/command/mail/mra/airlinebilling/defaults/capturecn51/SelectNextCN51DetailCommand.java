/*
 * SelectNextCN51DetailCommand.java Created on DEC 05, 2008
 ...
 *
 */
public class SelectNextCN51DetailCommand extends BaseCommand{

	private Log log = LogFactory.getLogger("MAILTRACKING MRA AIRLINEBILLING DEFAULTS");
	private static final String CLASS_NAME = "SelectNextCN51DetailCommand";
	...
	public void execute(InvocationContext invocationContext) throws CommandInvocationException {
		...
		int lastPageNum = Integer.parseInt(captureCN51Form.getLastPageNum());
		...
		if(displayPageNum == lastPageNum) {
			if(selectedCN51VOs != null && selectedCN51VOs.size() > 0) {
				...
				currentCN51VO = ((ArrayList<AirlineCN51DetailsVO>)
						selectedCN51VOs).get(selectedCN51VOs.size()-1);
			}
		}else if((displayPageNum < lastPageNum) && (displayPageNum  == 1)) {
			if(selectedCN51VOs != null && selectedCN51VOs.size() > 0) {
				currentCN51VO = new AirlineCN51DetailsVO();
				currentCN51VO = ((ArrayList<AirlineCN51DetailsVO>)selectedCN51VOs).get(0);
			}
		}else {
			if(selectedCN51VOs != null && selectedCN51VOs.size() > 0) {
				currentCN51VO = new AirlineCN51DetailsVO();
				currentCN51VO = ((ArrayList<AirlineCN51DetailsVO>)selectedCN51VOs).get(displayPageNum-1);
			}
		}
