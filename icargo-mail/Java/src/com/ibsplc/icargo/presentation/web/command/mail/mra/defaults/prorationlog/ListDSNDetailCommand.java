...
public class ListDSNDetailCommand extends BaseCommand {
	...
	private static final String KEY_INVALID_MAIL_DIPATCHID = "mailtracking.mra.defaults.despatchenquiry.invalidid";
	/**
	 * Execute method
	 * 
	...
	public void execute(InvocationContext invocationContext)
		...
		MRAProrationLogForm prorationLogForm = (MRAProrationLogForm) invocationContext.screenModel;
		...
		String dsnNumber = prorationLogForm.getDsn();
		String dsnDate = "";
		String fromScreen = "ProrationLog";
		int pageNumber = 1;
		...
		if(dsnNumber!=null && dsnNumber.trim().length()>0){
			...
			if(codeLength == 4 || codeLength == 20 || codeLength == 29 || codeLength == 12) {
				dsnNumber = dsnNumber.toUpperCase();
				dsnPopUpFilterVO.setDsn(dsnNumber);
			}
			else{
				errors = new ArrayList<ErrorVO>();
				errors.add(new ErrorVO(KEY_INVALID_MAIL_DIPATCHID));
		...
		if (despatchLovVOs != null && despatchLovVOs.size() > 0) {
			...
			if (despatchLovVOs.size() == 1) {
				...
				if (dsnPopUpVO.getBillingDetailsCount() == 0) {
					...
					invocationContext.addAllError(errors);
					invocationContext.target = LIST_FAILURE;
					prorationLogForm.setShowDsnPopUp(FLAG_FALSE);
				...
				prorationLogForm.setShowDsnPopUp(FLAG_TRUE);
