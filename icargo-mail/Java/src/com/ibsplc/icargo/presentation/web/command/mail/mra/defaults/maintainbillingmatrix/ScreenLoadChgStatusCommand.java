...
public class ScreenLoadChgStatusCommand extends BaseCommand {
	...
	public void execute(InvocationContext invocationContext)
		...
		// BillingMatrixForm
		// form=(BillingMatrixForm)invocationContext.screenModel;
		...
		if (session.getOneTimeVOs() == null) {
			...
			if (!isOneTimePresent) {
				Map<String, Collection<OneTimeVO>> oneTimeValues = fetchOneTimeDetails(logonAttributes
						.getCompanyCode());
				session.getOneTimeVOs().put(STATUS_ONETIME,
						oneTimeValues.get(STATUS_ONETIME));
			}
		}
