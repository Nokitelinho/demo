...
public class InvoiceClearancePeriodPrintCommand extends AbstractPrintCommand {
	 ...
	 * report id
	 */
	private static final String REPORT_ID = "RPTMRA019";
	/**
	 * bundle
	 */
...
//	private static final String INVALID_DATE = "cra.flown.msg.err.invaliddate";
	...
	private Collection<ErrorVO> validateForm(
		...
		String validationTempclr = null;

		String companyCode = getApplicationSession().getLogonVO().getCompanyCode();

		filterVO.setCompanyCode(companyCode);
		filterVO.setInterlineBillingType(form.getBillingType());
		filterVO.setIataClearancePeriod(form.getRpt019ClrPrd());
		validationTempclr = form.getRpt019ClrPrd();
		...
		iatacalendarfiltervo.setClearancePeriod(form.getRpt019ClrPrd());
		if (validationTempclr != null && validationTempclr.trim().length() > 0) {
			try {
		...
		if (iatacalendarvo != null ) {
			filterVO.setIataClearancePeriod(validationTempclr);
		...
		validationTemp = form.getRpt019AirlineCode();
		...
		validationTemp = form.getRpt019AirlineNum();
		if(!BLANK.equals(validationTemp)){
