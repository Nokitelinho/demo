/*
 * TransfermailAcknowledgeCommand.java Created on Oct 08, 2008
 *
 ...
 *
 */
public class TransfermailAcknowledgeCommand extends BaseCommand {
	...
	public void execute(InvocationContext invocationContext)
		...
		log.entering("TransfermailAcknowledgeCommand","execute");
		...
		String[] primKeyArr = uploadMailForm.getSelectTransfer();
		int scannedVOSize= scannedTransferVOs.size();
		for(int i=0;i<scannedVOSize;i++){
		scannedMailDetailsVO = ((ArrayList<ScannedMailDetailsVO>)scannedTransferVOs).get(i);
		Collection<MailbagVO> mailbagtransfervos = scannedMailDetailsVO.getMailDetails();
		
		MailbagVO mailbagVO = new MailbagVO();
		for(int cnt=0;cnt<primKeyArr.length;cnt++){
			int bag = Integer.parseInt(primKeyArr[cnt]);
			mailbagVO = ((ArrayList<MailbagVO>)mailbagtransfervos).get(bag-1);
		...
		log.exiting("TransfermailAcknowledgeCommand","execute");
