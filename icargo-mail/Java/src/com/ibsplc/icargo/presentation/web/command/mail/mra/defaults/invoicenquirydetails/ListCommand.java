...
public class ListCommand extends BaseCommand {
	...
	throws CommandInvocationException {
		...
		try {
			...
			mailInvoicEnquiryDetailsVOs = delegate.findInvoicEnquiryDetails(invoiceEnquiryFilterVo);
		...
		if (mailInvoicEnquiryDetailsVOs !=null&&mailInvoicEnquiryDetailsVOs.size()>0
				|| mailInvoicEnquiryDetailsVOs !=null && mailInvoicEnquiryDetailsVOs.size()>0) {
			MailInvoicEnquirySummaryVO summaryVO = new MailInvoicEnquirySummaryVO();
			summaryVO.setPoaCode(mailInvoicEnquiryDetailsVOs.get(0).getPoaCode())  ;
			summaryVO.setScheduledInvoiceDate(mailInvoicEnquiryDetailsVOs.get(0).getScheduledInvoiceDate());
			summaryVO.setPaymentType(mailInvoicEnquiryDetailsVOs.get(0).getPaymentType());
			summaryVO.setReconcilStatus(mailInvoicEnquiryDetailsVOs.get(0).getReconcilStatus());
			summaryVO.setCarrierCode(mailInvoicEnquiryDetailsVOs.get(0).getCarrierCode());
			summaryVO.setCarrierName(mailInvoicEnquiryDetailsVOs.get(0).getCarrierName());
			summaryVO.setControlValue(mailInvoicEnquiryDetailsVOs.get(0).getControlValue());
			
			summaryVO.setTotalAdjustmentAmount(mailInvoicEnquiryDetailsVOs.get(0).getTotalAdjustmentAmount());
