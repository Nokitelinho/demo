/*
 * AddMailBagsCommand.java Created on Jul 1 2016
 *
 ...
 * 
 */
public class AddMailBagsCommand extends BaseCommand {
	...
	public void execute(InvocationContext invocationContext)
		...
		log.entering("AddMailBagsCommand", "execute");
		...
		ContainerDetailsVO containerDetailsVO = mailAcceptanceSession
				.getContainerDetailsVO();
		...
		String container = "";
		String mailId = "";
		Collection<MailbagVO> firstMailbagVOs = containerDetailsVO
				.getMailDetails();
		MailAcceptanceVO mailAcceptanceVO = mailAcceptanceSession
				.getMailAcceptanceVO();
		Collection<ContainerDetailsVO> containerDetailsVOs = mailAcceptanceVO
				.getContainerDetails();
			...
			if (firstMailbagVOs != null && firstMailbagVOs.size() > 0) {
				for (MailbagVO fstMailbagVO : firstMailbagVOs) {
					for (ContainerDetailsVO popupVO : containerDetailsVOs) {
						if (!fstMailbagVO.getContainerNumber().equals(
								popupVO.getContainerNumber())) {
							Collection<MailbagVO> secMailVOs = popupVO
									.getMailDetails();
							if (secMailVOs != null && secMailVOs.size() > 0) {
								for (MailbagVO secMailbagVO : secMailVOs) {
									if (fstMailbagVO.getMailbagId().equals(
											secMailbagVO.getMailbagId())) {
										duplicateAcross = 1;
										container = popupVO
												.getContainerNumber();
		...
		if (duplicateAcross == 1) {
			invocationContext
					.addError(new ErrorVO(
							"mailtracking.defaults.mailacceptance.duplicatemailbagacross",
							new Object[] { container, mailId }));
			invocationContext.target = TARGET;
			log.exiting(
					"AddMailBagsCommand - duplicate mailbags across containers",
					"execute");
			return;
		}
		...
		if (sameOE != 1) {
			...
			/**
			 * To get the last vo values.New row to be defaulted with this VO
			 * values.
			 */
			if (mailbagVOs != null && mailbagVOs.size() > 0) {
			...
			String year = (new LocalDate(logonAttributes.getAirportCode(),
					Location.ARP, true)).toDisplayDateOnlyFormat();
			newMailbagVO.setYear(Integer.parseInt(year.substring(
					year.length() - 1, year.length())));
			newMailbagVO.setPou(containerDetailsVO.getPou());
			newMailbagVO.setScannedDate(new LocalDate(logonAttributes
			...
			if (nwMailVO != null) {
				...
				newMailbagVO
						.setMailCategoryCode(nwMailVO.getMailCategoryCode());
				newMailbagVO.setMailSubclass(nwMailVO.getMailSubclass());
				newMailbagVO.setYear(nwMailVO.getYear());
				newMailbagVO.setDespatchSerialNumber(nwMailVO
						.getDespatchSerialNumber());
				newMailbagVO.setHighestNumberedReceptacle(nwMailVO
						.getHighestNumberedReceptacle());
				newMailbagVO.setRegisteredOrInsuredIndicator(nwMailVO
		...
		invocationContext.target = TARGET;
		log.exiting("AddMailBagsCommand", "execute");
