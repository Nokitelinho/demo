...
public class ListManifestCommand extends AbstractCommand{

	private Log log = LogFactory.getLogger("ListManifestCommand");
	...
	public void execute(ActionContext actionContext) throws BusinessDelegateException {
		log.entering("ListManifestCommand", "execute");
		...
		if (awbFilter.getManifestDateFrom() == null || awbFilter.getManifestDateFrom().trim().length() == 0) {
			awbFilter.setManifestDateFrom((new LocalDate(logonAttributes.getAirportCode(), Location.ARP, true).addDays(-30))
					.toDisplayDateOnlyFormat());
		}

		if (awbFilter.getManifestDateTo() == null || awbFilter.getManifestDateTo().trim().length() == 0) {
			awbFilter.setManifestDateTo(
					new LocalDate(logonAttributes.getAirportCode(), Location.ARP, true).toDisplayDateOnlyFormat());
		}
		if(!awbFilter.isOnListButtonClick()){
			awbFilter.setPou(logonAttributes.getStationCode());
		}
		mailBookingFilterVO = MailOperationsModelConverter.constructManifestFilterVO(awbFilter, logonAttributes);
		...
		Page<MailBookingDetailVO> manifestDetailVos = null;
		AddonsMailDelegate delegate = new AddonsMailDelegate();
		try {
			manifestDetailVos = delegate.findManifestBookings(mailBookingFilterVO, Integer.parseInt(displayPg));
			...
			mailBookingDetails = MailOperationsModelConverter.convertManifestDetailVOsToModel(manifestDetailVos);
			PageResult<MailBookingDetail> manifestDetailsCollectionPage = 
					new PageResult<>(manifestDetailVos,(ArrayList<MailBookingDetail>) mailBookingDetails);
			mailAwbBookingModel.setManifestFilter(awbFilter);
			mailAwbBookingModel.setManifestDetailsCollectionPage(manifestDetailsCollectionPage);
