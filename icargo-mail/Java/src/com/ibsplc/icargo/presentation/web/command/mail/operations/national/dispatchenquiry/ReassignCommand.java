/**
 * ReassignCommand. Created on February 22, 2012
 *
...
public class ReassignCommand extends BaseCommand{
	...
	private static final String CONST_REASSIGNDISPATCH = "REASSIGN";
	...
	throws CommandInvocationException {
		...
		Collection<DespatchDetailsVO> despatchVOSToReassign = new ArrayList<DespatchDetailsVO>();
		...
		if(("I").equals(dispatchEnquiryForm.getOperationType())){
			...
			"mailtracking.defaults.national.reassign.error.cannotReassignInboundDsn");
		...
		for (DespatchDetailsVO despatchDetailsVO : despatchDetailsVOs) {
			for (int j = 0; j < selectedRows.length ; j++) {
				if (row == Integer.parseInt(selectedRows[j])) {    			
					despatchVOSToReassign.add(despatchDetailsVO);
		...
		for(DespatchDetailsVO despatchDetailsVO : despatchVOSToReassign){
		...
		reassignSession.setDespatchDetailsVOs(despatchVOSToReassign);
		dispatchEnquiryForm.setPopupFlag(CONST_REASSIGNDISPATCH);
		invocationcontext.target = SCREENLOAD_SUCCESS;
