/*
 * ValidateAttachRoutingCommand.java Created on Jul 1 2016
 *
...
public class ValidateAttachRoutingCommand extends BaseCommand {
	...
	throws CommandInvocationException {
		...
		String csgDocNum = "";
		String paCode = "";
		mailManifestForm.setCsgDocNumForRouting("");
		mailManifestForm.setPaCodeForRouting("");
		/*
		 * Selected Container(s)if any
		 */
		if (mailManifestForm.getParentContainer()!=null && mailManifestForm.getParentContainer().length()>0){
		...
		if (csgDocNumDiff == 0 && routingAvl == 0 && paCodeDiff == 0){
			if(csgDocNum != null && csgDocNum.length() > 0 && 
				...
				mailManifestForm.setCsgDocNumForRouting(csgDocNum);
				mailManifestForm.setPaCodeForRouting(paCode);
			}
		}
		...
		/*
		 * Creating MailInConsignmentVOs and setting to Session
		 */
		mailManifestSession.setMailVOs(createMailInConsignmentVOs(containerDetailsVOs));
		
		mailManifestForm.setAttachRouting("OK");
		mailManifestForm.setScreenStatusFlag(ComponentAttributeConstants.SCREEN_STATUS_DETAIL);
