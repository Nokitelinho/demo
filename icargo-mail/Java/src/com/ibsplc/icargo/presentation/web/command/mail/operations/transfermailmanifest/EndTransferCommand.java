...
public class EndTransferCommand extends BaseCommand {
	 private Log log = LogFactory.getLogger("MAILTRACKING");
	   ...
	   private static final String TARGET_SUCCESS= "transfer_completed";
	   private static final String TARGET_FAILURE = "transfer_failed";
	...
	public void execute(InvocationContext invocationContext) throws CommandInvocationException {
		TransferMailManifestForm transferMailManifestForm = 
			...
			Collection<ErrorVO> formErrors = new ArrayList<>();
			...
			for(TransferManifestVO transferManifest:transferManifestVOList){
				if(!"TRFREJ".equals(transferManifest.getTransferStatus())){//partial transfer
			 try {
				new MailTrackingDefaultsDelegate().saveTransferFromManifest(transferManifest)	;
			} catch (BusinessDelegateException e) {
				e.getMessage();
			      }
			    }
	    	...
	    	log.exiting("EndTransferCommand","execute");
