...
public class InvoiceByAirlinePrintCommand extends AbstractPrintCommand {
	 ...
	 * report id
	 */
	private static final String REPORT_ID = "RPTMRA013";
	/**
	 * bundle
	 */
...
//	private static final String INVALID_DATE = "cra.flown.msg.err.invaliddate";
	...
	private Collection<ErrorVO> validateForm(
		...
		validationTemp = form.getRpt013FromDate();
		...
		validationTemp = form.getRpt013ToDate();
		if(!BLANK.equals(validationTemp)){
		...
		validationTemp = form.getRpt013AirlineCode();
		if(!BLANK.equals(validationTemp)){
		...
		validationTemp = form.getRpt013AirlineNum();
		if(!BLANK.equals(validationTemp)){
